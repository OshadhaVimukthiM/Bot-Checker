...
-> 100 CREDITS + PREMIUM ACCESS - 2$
-> 300 CREDITS + PREMIUM ACCESS - 4$
-> 500 CREDITS + PREMIUM ACCESS - 6$
-> 1000 CREDITS + PREMIUM ACCESS - 10$
>> PING <code>@OshadhaVimukthi</code> For Purchasing
Note -⟩ We Only Accept Paypal And Crytpo</b>");

$nopre = urlencode("<b>YOU NEED TO BE PREMIUM TO USE THIS COMMAND.
Hit /buy to purchase</b>");
$botToken =  "2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk";
...
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
-> 500 CREDITS + PREMIUM ACCESS - 6$
-> 1000 CREDITS + PREMIUM ACCESS - 10$
Note-⟩ We Only Accept [Paypal][GIFT CARDS][CRYTPO]</b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
file_get_contents("https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/editMessageText?chat_id=$cchatid2&text=$finalize&message_id=$cmessage_id2&parse_mode=HTML");
}
...
THIS BOT IS MADE WITH ♥️ BY  <code>@OshadhaVimukthi</code> </b>",
 'parse_mode'=>'HTML',
 'reply_markup' => $free,
 ]);
        exit();
...
$premium_id = (array("1382910410","1254478266"));
$premiumgp = (array("-1001509666768","-1001421888750"));
        $keyboard = json_encode([
'inline_keyboard' => [
[['text' => "OWNER", 'url' => "https://t.me/OshadhaVimukthi"],]
]]);
                $keyboard1 = json_encode([
'inline_keyboard' => [
[['text' => "CHANNEL", 'url' => "https://t.me/paidtricksandbins"],
['text' => "GROUP", 'url' => "https://t.me/paidtricksandbinschat"],]
]]);

// if(!in_array($chatId, $premiumgp)){
	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/getChatMember?chat_id=@paidtricksandbinschat&user_id=".$gId))->result->status;
...
// 
// Made With ♥️ By @OshadhaVimukthi
...
// exit();
// }
        	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/getChatMember?chat_id=@roldexversechats&user_id=".$gId))->result->status;
// if($tch != 'member' && $tch != 'creator' && $tch != 'administrator'){
// bot('sendMessage',['chat_id'=>$chatId,'reply_to_message_id'=>$message_id,'text'=>"Sorry <a href='tg://user?id=$gId'>$firstname</a>
// 
// You Need To Join My Update Channel To Get Regular Updates
...
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot: @ShadowCheckerbot </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot: @ShadowCheckerbot </b>");
}else{
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A✅LIVE KEY✅</b>%0A<u>KEY:</u> <code>${fii}xxxxxxxxxxxxxxxxx${newstring}</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot: @ShadowCheckerbot</b>");
// sleep(10);
...
function bot($method,$datas=[]){
    $url = "https://api.telegram.org/bot2105189127:AAFr1gJspzA43gjLqMqjAmx-4IZmSRlYPvk/".$method;
    $ch = curl_init();
...
global $botToken;
$url = "https://api.telegram.org/bot".$botToken."/editMessageText?chat_id=".$chatId."&message_id=".$message_id."&text=••MASS CHECKER%0A".$message."%0A••• BOT BY: @OshadhaVimukthi&parse_mode=HTML";
...
<i>take group access from <code>@OshadhaVimukthia</code> to use here</i></b>");
